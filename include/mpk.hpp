 ...
 *        two roofs on s.
 *
 * Tiling is what earns the vertical axis: a naive kernel just streams the operator s times,
 * so instead the rows are split into panels and all s powers are computed while a panel
 * stays resident. Each panel carries an s*w-wide halo (w = operator bandwidth), halos are
 * shared within a band, so they cost footprint, not arithmetic.
 *
 * Two independent roofs bound s: a CAPACITY roof (panel stops fitting or halo stops being
 * thin) and a NUMERICAL roof (certified s_max). The operative s is the smaller.
 * The pattern switch decides a priori from the scatter knob b whether tiling can pay:
 * a thin halo at  b = 1 (banded), a whole-vector halo at b = N (scattered, falls back to naive s-SpMV).
 *
 * Everything here is a predictor. measured_bandwidth() only checks predicted_bandwidth().
 *
 ...
 * whole vector, which is why the tiled kernel has nothing to tile there).
 *
 * An UPPER bound, the safe direction for a switch: it can only refuse to tile where tiling
 * was viable, never tile where it cannot pay. measured_bandwidth() checks it.
 */
 ...
 */
[[nodiscard]] inline double tile_capacity_bytes(const Machine& mc, TileLevel level) noexcept
{
    if (level == TileLevel::L2) return static_cast<double>(mc.l2_bytes_core);
    return static_cast<double>(mc.llc_slice_bytes)
         / static_cast<double>(mc.cores_per_llc_slice);
}
...
[[nodiscard]] inline int64_t mpk_panel_rows(const Machine& mc, TileLevel level, int s,
                                            int64_t w, int64_t n, double nnz_per_row) noexcept
{
    const double capacity = tile_capacity_bytes(mc, level);
    const double ds  = static_cast<double>(s);
    const double dw  = static_cast<double>(w);
...
[[nodiscard]] inline double mpk_tiled_R_v(const Machine& mc, TileLevel level, int64_t rows,
                                          ...
                                          double nnz_per_row) noexcept
{
    return mpk_panel_bytes(rows, s, w, n, nnz_per_row) / tile_capacity_bytes(mc, level);
}
...
[[nodiscard]] inline int s_ghost_max(const Machine& mc, TileLevel level, int64_t w, int64_t n,
                                     double nnz_per_row, int s_ceiling) noexcept
{
    int best = 0;
    for (int s = 1; s <= s_ceiling; ++s) {
        const int64_t rows = mpk_panel_rows(mc, level, s, w, n, nnz_per_row);
                                      ...
                                      double nnz_per_row, int s_ceiling = 64)
{
    ...
    p.s_capacity  = s_ghost_max(mc, level, p.w, n, nnz_per_row, s_ceiling);
    p.s           = operative_s(p.s_capacity, s_certified);
    p.capacity_binds = (p.s_capacity < s_certified);

    p.panel_rows = mpk_panel_rows(mc, level, p.s, p.w, n, nnz_per_row);
    if (p.panel_rows <= 0) {
