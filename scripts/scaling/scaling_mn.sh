...
#
# The scaling study freezes the Krylov dimension at m=8 so the kernel mix does not drift.
# That quarantines, but does not hide, the fact that the true per-unknown Krylov cost m(n)
# grows under grid refinement: as h ~ 1/n shrinks, the spectrum of the discretized
# generator widens, so exp(A~)v needs more Arnoldi steps to hold tolerance. This sweep
# measures that growth with the genuine convergence-driven KSM-EI (via the profiler's
# avg_krylov) and reports it as an independent result that itself motivates CA: more
