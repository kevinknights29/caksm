...
#
# Work-per-core is held (approximately) constant by growing the problem with the
# core count. Since N ~ n^3, the schedule is n(P) = n1 * P^(1/3) rounded to the
# nearest ODD integer (odd is a hard grid constraint). Because n is rounded to
# odd integers, N/P cannot be held EXACTLY constant. The achieved ratio is
# recorded per point (aug_N and P columns in the CSV) so the reader sees the seam.
#
#   P    n (odd)   role
#   1    31        anchor
#   5    53
...
# presentation is weak-scaling EFFICIENCY T(1)/T(P), a decay from 1.0, and is
# produced by scripts/scaling_plot.py. SpMV crosses into DRAM around n ~ 39 and
# GS around n ~ 57 mid-sweep. Those tier crossings are annotated on the plot.
#
